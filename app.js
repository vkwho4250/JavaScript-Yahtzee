    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        root.style.setProperty(
          '--player-color',
          `var(--neon-${playerColor[2]})`,
        );
        setTimeout(() => {
          root.style.setProperty(
            '--player-color',
            `var(--neon-${playerColor[3]})`,
          );
...
playerNumOptions.forEach((el) => {
  el.addEventListener('click', (e) => {
    numOfPlayers = parseInt(e.currentTarget.id[10], 10);
...
function initialize(numberOfPlayers) {
  // creates Player object for each player and adds to player array
  for (let i = 0; i < numberOfPlayers; i++) {
    playerArray.push(new Player(i + 1, playerColor[i]));
  }
    ...
    player.scoreDisplay.forEach((el) => {
      el.addEventListener('click', (e) => {
        if (turnStarted && !gameEnded) {
          for (let i = 0; i < 13; i++) {
            if (
              e.target === currentPlayer.scoreDisplay[i] &&
              !currentPlayer.selectedScores[i]
            ) {
              ...
              totalScoreElement[currentPlayer.number - 1].textContent =
                currentPlayer.scoreTotal;
              rollCount++;
              resetTurn();
              break;
            }
  ...
  diceCardBtns.forEach((el) => {
    el.addEventListener('click', () => {
      if (turnStarted && !gameEnded) {
        const cardLocked = parseInt(el.id[4], 10) - 1;
...
menuToggle.addEventListener('click', () => {
  if (menuContainer.classList.length === 0) {
    menuContainer.classList.add('open1');
  } else {
...
document.querySelector('#instructions-btn').addEventListener('click', () => {
  ...
  document
    .querySelector('.instructions-response')
...
function resetTurn() {
  ...
  if (roundNum === 13 && playerNum === numOfPlayers) gameOver();
    ...
    currentPlayer.selectedScores.forEach((el, index) => {
      if (!el) currentPlayer.scoreDisplay[index].textContent = 0;
    });
...
function calcScore() {
  ...
  const sortedResults = [...new Set(diceResults)]; // removes duplicates
  sortedResults.sort((a, b) => a - b); // sorts dice values from small to large

  if (
    sortedResults.length === 5 &&
    isEqual(bigStraight[sortedResults[0] - 1], sortedResults)
  ) {
    newScoreArray[9] = 30;
    newScoreArray[10] = 40;
  } else if (
    sortedResults.length >= 4 &&
    (isEqual(littleStraight[sortedResults[0] - 1], sortedResults.slice(0, 4)) ||
      isEqual(littleStraight[sortedResults[1] - 1], sortedResults.slice(1, 5)))
  ) {
    newScoreArray[9] = 30;
    newScoreArray[10] = 0;
  } else {
...
/* updates scoreboard with new calculated scores only if category has
   not been already selected/accounted for */
